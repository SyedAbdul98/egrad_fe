...
const TDiscussion = () => {
  ...
  const handleSendMessage = (e) => {
    ...
    console.log("s", senderName, "c", courseName, "m", message);
    sendMessage();
  };
  ...
  useEffect(() => {
    console.log("message");
    console.log("u", user);
    setSenderName(user?.teacherName);
    ...
    <div className=" h-screen relative">
      ...
      <div className=" overflow-auto " style={{ height: "84vh" }}>
        {discussions.map((msg, index) =>
          msg.senderName == user.teacherName ? (
            <div
              className="flex flex-wrap flex-col m-2 p-1 h-max w-max rounded justify-start float-right clear-both relative shadow-lg bg-indigo-200"
              key={index}
            >
